...
export class UpgradeScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .text(width / 2, height / 4, "업그레이드 선택", {
        fontSize: "32px",
        color: "#fff",
        fontFamily: "Inter",
      })
    ...
    const upgradeOptions = [
      {
        name: "공격력 증가",
        effect: () => (GameState.playerStats.attackDamage += 5),
      },
      {
        name: "공격 속도 증가",
        effect: () => (GameState.playerStats.attackRate *= 0.9),
      },
      {
        name: "최대 체력 증가",
        effect: () => {
      ...
      {
        name: "이동 속도 증가",
        effect: () => (GameState.playerStats.speed += 20),
      },
    ];
    ...
    selected.forEach((option, index) => {
      ...
      const text = this.add
        .text(0, 0, option.name, { fontSize: "18px", color: "#fff" })
        .setOrigin(0.5);
